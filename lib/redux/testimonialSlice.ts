  ...
  async (params: QueryParams = {}, { rejectWithValue }) => {
    try {
      ...
      if (visible !== undefined) {
        queryParams.append('visible', visible.toString())
      }
      
      const endpoint = `/api/testimonials?${queryParams.toString()}`
...
export const createTestimonial = createAsyncThunk(
  ...
  async (testimonialData: TestimonialRequest, { rejectWithValue }) => {
    try {
      const response = await apiService.post('/api/testimonials', testimonialData)
      return response
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || error.message)
    }
...
export const updateTestimonial = createAsyncThunk(
  ...
  async ({ id, testimonial }: { id: string; testimonial: Partial<TestimonialRequest> }, { rejectWithValue }) => {
    try {
      const response = await apiService.put(`/api/testimonials/${id}`, testimonial)
      return response
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || error.message)
    }
...
export const toggleTestimonialVisibility = createAsyncThunk(
  ...
  async ({ id, isVisible }: { id: string; isVisible: boolean }, { rejectWithValue }) => {
    try {
      const response = await apiService.put(`/api/testimonials/${id}`, { isVisible })
      return response
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || error.message)
    }
...
export const { clearError, clearCurrentTestimonial, setCurrentPage } = testimonialsSlice.actions
export default testimonialsSlice.reducer

// Selectors
export const selectTestimonials = (state: { testimonials: TestimonialsState }) => state.testimonials.testimonials
export const selectCurrentTestimonial = (state: { testimonials: TestimonialsState }) => state.testimonials.currentTestimonial
...
export const selectTestimonialsPagination = (state: { testimonials: TestimonialsState }) => ({
  total: state.testimonials.total,
  page: state.testimonials.page,
  totalPages: state.testimonials.totalPages,
})
export const selectTestimonialsActionStates = (state: { testimonials: TestimonialsState }) => ({
